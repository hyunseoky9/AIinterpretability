...
def calc_performance_gap(envsetting):
    # setting parameters
    ...
    pickle_filenames, episodes = load_episodes(envsetting)
    ...
    for epi, ep in enumerate(episodes):
        ...
        for simstep in simsteps:
            ...
            ep['L1_a_dist_RLsampled_a_prob_R'].append(np.abs(stop_accounted_aR - ep['RLactions_prob'][simstep][0:env.patchnum]).sum())
            ep['L1_a_dist_RLsampled_a_prob_S'].append(np.abs(stop_accounted_aS - ep['RLactions_prob'][simstep][env.patchnum:]).sum())
...
if __name__ == "__main__":
    envsetting = 21 # 18(n5), 20(n10), 21(n20) 
        
    t0 = time.time()
    calc_performance_gap(envsetting)
